...
def update_download_page_defaults(page):
    ...
    if len(attachments) > 1:
        ...
        prefix = ", ".join(file_descriptions[:3]) + (
            ", etc." if len(file_descriptions) > 3 else ""
        )
        ...
        if document:
            prefix = (
                ...
                if attachments[0].title
                    ...
                    document.title
                    if document.title
                    else document.filename if document.filename else "Document inconnu"
                )
            )
            prefix = prefix[:20] + ("..." if len(prefix) > 20 else "")
            extension = document.filename.split(".")[-1]
            suffix = (
                f"{round(document.file.size / 1000)} Ko"
                if document.file.size < 100000
                else f"{round(document.file.size / 1000000, 2)} Mo"
            )
            if extension in FILE_EXTENSIONS:
                file_type, file_desc = FILE_EXTENSIONS[extension]
                new_title = (
                    f"[{prefix}]-[{file_desc if file_desc else 'inconnu'}]-[{suffix}]"
                )
    ...
    if (
        page.slug != new_slug
        or page.title != new_title
        or page.heading != new_heading
        or page.tooltip != new_tooltip
        or page.logo != icon
    ):
        page.slug = new_slug
        page.title = new_title
...
def custom_process_form(page, request, *args, **kwargs):
    ...
    if isinstance(page, AmicaleInscriptionPage):
        if user.groups.filter(name="Amicale").exists():
            messages.warning(
                request, _("You are already an amicale member."), fail_silently=True
            )
        ...
        if not user.groups.filter(name="Amicale").exists():
            messages.warning(
                request,
                _(
                    "You have to be an amicale member to sign in for this event. Hopefully you can click on the 📝 in the top right corner to sign up."
                ),
                fail_silently=True,
            )
    ...
    if unique:
        if (
            form_def.get_submission_class()
            .objects.filter(
                form_id=form_def.id, form_data__contains=f'"userid": "{user.id}"'
            )
            .exists()
        ):
            print(
                colored(
                    "Un doublon existe déjà pour cet utilisateur et ce formulaire.",
                    "red",
                )
            )
            messages.warning(
                request, _("You have already submitted this form."), fail_silently=True
            )
            return redirect("/amicale/")
    ...
    if form.is_valid():
        ...
        if form_def.success_message:
            ...
            messages.success(
                request, "Votre demande a bien été enregistrée.", fail_silently=True
            )
